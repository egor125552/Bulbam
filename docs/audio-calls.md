...
- `UserRealtime` remains the per-user WebSocket fanout channel for live events and also owns one atomic active-call slot for that user.
...
The per-user active-call slot prevents a user from being put into two calls from two different conversations at the same time. Starting a call first claims both participants' slots; if either participant is already busy, the other temporary claim is rolled back and the new call is rejected with `call_busy`.
...
4. The server atomically reserves one active-call slot for each participant.
5. The conversation's CallRoom creates a `ringing` call and publishes `call.ringing` through the recipient's `UserRealtime` Durable Object.
6. If the recipient is not in the foreground and Web Push is configured, the server also sends an incoming-call push deep-linked to both the conversation and the call.
7. The recipient presses **Ответить**. CallRoom transitions the call from `ringing` to `accepted` and publishes `call.answered`.
8. The caller creates the SDP offer, the recipient creates the SDP answer, and both sides exchange trickled ICE candidates through CallRoom.
9. CallRoom forwards each signal immediately through realtime and retains only a bounded recent sequence so a short WebSocket interruption can recover by HTTP polling.
10. Either participant can end an accepted call. Only the recipient can decline a ringing call.
...
- A separate atomic slot in each participant's UserRealtime prevents simultaneous calls across different conversations.
...
- Ending or declining clears the retained signaling buffer and releases both user call slots.
...
- Only the callee can submit the WebRTC answer or request `resume` after that side reloads.
- Either participant can submit ICE candidates after acceptance and either can hang up.
- Internal CallRoom and active-call-slot routes are reached only through Durable Object bindings; browsers never receive their internal URLs.
...
Like Lenovo Messenger's active-call restore, UserRealtime keeps a short-lived pointer to the current call. Whenever a browser establishes a new realtime WebSocket, the server sends `call.active` after `realtime.ready` if a call is still alive.

A full page reload deliberately does **not** silently reopen the microphone. The web client shows **Восстановить звук**. After that explicit user action it reacquires the microphone, skips stale signaling already retained in CallRoom, and starts a fresh WebRTC negotiation. If the callee was the side that reloaded, it emits `resume`; the caller responds with a fresh ICE-restart offer. If the caller reloaded, it can directly create the fresh offer after the restore action.

## Push and accessibility

Incoming call Web Push points to `?chat=<conversationId>&call=<callId>`. After authentication the PWA can restore the CallRoom state and present **Ответить** / **Отклонить**. An already accepted call instead presents **Восстановить звук** so microphone access always follows an explicit user action.
