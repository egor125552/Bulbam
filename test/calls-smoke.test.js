...
describe("one-to-one audio call signaling", () => {
  test("authorizes ringing, answer, WebRTC signaling, busy state and hangup", async () => {
    ...
    expect((await json(ice)).iceServers[0].urls).toContain("stun:stun.cloudflare.com:3478");

    const started = await api(`/api/v1/chats/${chat.conversationId}/calls`, alpha.cookie, { method: "POST" });
    ...
    const betaView = await api(`/api/v1/calls/${call.callId}`, beta.cookie);
    ...
    const callerCannotAnswer = await api(`/api/v1/calls/${call.callId}/answer`, alpha.cookie, { method: "POST" });
    await expectStatus(callerCannotAnswer, 403);

    const busy = await api(`/api/v1/chats/${chat.conversationId}/calls`, beta.cookie, { method: "POST" });
    await expectStatus(busy, 409);

    const answered = await api(`/api/v1/calls/${call.callId}/answer`, beta.cookie, { method: "POST" });
    await expectStatus(answered, 200);
    expect((await json(answered)).call.status).toBe("accepted");

    const calleeCannotOffer = await api(`/api/v1/calls/${call.callId}/signals`, beta.cookie, {
      method: "POST",
      body: JSON.stringify({
        kind: "offer",
    ...
    const offerResponse = await api(`/api/v1/calls/${call.callId}/signals`, alpha.cookie, {
      method: "POST",
      body: JSON.stringify({
        kind: "offer",
        payload: { type: "offer", sdp: "v=0\r\nmock-offer" }
    ...
    const signalsForBeta = await api(`/api/v1/calls/${call.callId}/signals?after=0`, beta.cookie);
    ...
    const answerSignal = await api(`/api/v1/calls/${call.callId}/signals`, beta.cookie, {
      method: "POST",
      body: JSON.stringify({
        kind: "answer",
    ...
    const iceSignal = await api(`/api/v1/calls/${call.callId}/signals`, alpha.cookie, {
      method: "POST",
      body: JSON.stringify({
        kind: "ice",
        payload: {
    ...
    const signalsForAlpha = await api(`/api/v1/calls/${call.callId}/signals?after=${offer.sequence}`, alpha.cookie);
    await expectStatus(signalsForAlpha, 200);
    const alphaSignals = (await json(signalsForAlpha)).signals;
    expect(alphaSignals).toEqual(
    ...
    const ended = await api(`/api/v1/calls/${call.callId}/end`, beta.cookie, { method: "POST" });
    await expectStatus(ended, 200);
    expect((await json(ended)).call).toMatchObject({
      status: "ended",
      endedByUserId: beta.account.userId
    });

    const signalAfterEnd = await api(`/api/v1/calls/${call.callId}/signals`, alpha.cookie, {
      method: "POST",
      body: JSON.stringify({
        kind: "ice",
        payload: { candidate: "candidate:2 1 UDP 1 192.0.2.2 50001 typ host" }
      })
    });
    await expectStatus(signalAfterEnd, 409);

    const nextCall = await api(`/api/v1/chats/${chat.conversationId}/calls`, alpha.cookie, { method: "POST" });
